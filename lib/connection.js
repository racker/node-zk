...
ZkConnection.prototype._process_inbuf = function() {
  ...
  do {
    ...
    if (this._state === ZOO_CONNECTED_STATE) {
      if (this._inbuf.length >= ReplyHeaderFixedSize) {
...
ZkConnection.prototype._processReplyHeader = function(buf) {
  var rh = new ReplyHeader();
  rh.deserialize(buf);
  /* TODO: do things based on the reply header... */
