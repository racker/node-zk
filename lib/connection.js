...
function ZkConnection(port, host, options) {
  ...
  this._port = port;
  this._host = host;
  this._options = misc.merge(options, DEFAULT_OPTIONS);
...
ZkConnection.prototype._on_connect = function() {
  console.error('on_connect');
};

ZkConnection.prototype._on_data = function(data) {
  console.error('on_data', data);
};
...
ZkConnection.prototype._on_close = function(close) {
  console.error('on_close');
};
